...
function BrandReport() {
  ...
  const downloadPDF = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/car-brand/get-all-report`,
        {
          method: "GET",
          headers: {
  ...
  return (
    <section className="border rounded p-4 my-4 flex justify-end bg-white">
      ...
      <button
        type="button"
        className="relative inline-flex items-center space-x-2 px-6 py-2 text-sm font-medium rounded-md text-white bg-tm60 hover:bg-tm40"
        onClick={downloadPDF}
      >
