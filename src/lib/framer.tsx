    ...
    >
        ...
        return (
          <motion.button
            ...
            className={classNames(
              "text-lg relative rounded-md flex items-center h-10 px-4 z-20 bg-transparent cursor-pointer select-none transition-colors hover:text-black/20 mb-0.5",
              {
                "text-black/60": !isActive, // Default color for non-active tabs
                "text-black/100": isActive, // Color for active tabs
              }
            )}
      ...
      <AnimatePresence>
        {hoveredRect && navRect && (
          <motion.div
            key={"hover"}
            className="absolute p-1 mb-1 z-10 top-0 left-0 rounded-md bg-zinc-200 dark:bg-gray-100"
            initial={{
